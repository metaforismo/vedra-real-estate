...
export function marketView(s) {
  ...
  return `${pageHeading('RIFERIMENTI','Mercato e benchmark','Riferimenti importati e confronti omogenei, separati dai prezzi richiesti degli annunci.',s.user.role==='admin'?action('import','Importa benchmark','upload','btn primary'):'')}
    ...
    <section class="panel market-method"><h2>Quotazioni ufficiali in tutta Italia</h2><p>Consulta province, comuni e zone OMI. Periodo e fonte restano visibili; le quotazioni vengono conservate in cache e aggiornate alla scadenza.</p>${action('omi-open','Consulta OMI Italia','map','btn primary')}</section>
