...
def test_demo_real_source_mixing_rejected(api):
    _,c,_=api
    source={'name':'Test agency','domain':'catalog.example','config':{'search_url':'https://catalog.example/search'},'permission_confirmed':True,'permission_note':'Permesso test per mock locale'}
    response=c.post('/api/sources',json=source);assert response.status_code==201
    sid=response.json()['id']
    assert not response.json()['allowed_on_server']
    response=c.post('/api/agents',json={'name':'Misto','city':'Milano','source_ids':['demo-milano',sid]})
    assert response.status_code==422
    assert c.post(f'/api/sources/{sid}/probe').json()['ok'] is False
...
def test_exports_actual_office_documents(api):
    ...
    assert wb['Opportunità']['O3'].value=='SINTETICO / DEMO'
    ...
    assert 'DATI SINTETICI' in text and p['url'] in text and 'perizia' in text
    response=c.post('/api/export',json={'format':'csv','dataset':'demo','ids':[p['id']]})
    assert response.content.startswith(b'\xef\xbb\xbf') and 'SINTETICO / DEMO' in response.text
